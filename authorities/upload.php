  ...
  if(isset($_POST["submit"]) && !empty($_FILES["file"]["name"])){
    ...
    if(in_array($fileType, $allowTypes)){
      ...
      if(move_uploaded_file($_FILES["file"]["tmp_name"], $targetFilePath)){
        ...
        $insert = $connect->query("insert into producttable (pid, productname, productdescription, productprice, productqty, productdprice, productimagelink) values(NULL, '".$_REQUEST['productname']."', ".$_REQUEST['productdescription'].", ".$_REQUEST['productprice'].", ".$_REQUEST['productqty'].", ".$_REQUEST['productdprice'].", '".$fileName."')");
        if($insert){
