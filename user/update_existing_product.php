  ...
  if(!empty($_FILES["file"]["name"])){
    ...
    if(in_array($fileType, $allowTypes)){
      if(move_uploaded_file($_FILES["file"]["tmp_name"], $targetFilePath)){
        ...
        $query = "update producttable set producttitle=".$title.", productdescription='".$_REQUEST['productdescription']."', productprice=".$_REQUEST['productprice'].", productdprice=".$dprice.", productimagepath='".$fileName."' where productcode='".$_REQUEST['productcode']."'";
    ...
    $title = (empty($_REQUEST['producttitle']))? "NULL" : "'".$_REQUEST['producttitle']."'";
    $dprice = (empty($_REQUEST['productdprice']))? "NULL" : $_REQUEST['productdprice'];
    $query = "update producttable set producttitle=".$title.", productdescription='".$_REQUEST['productdescription']."', productprice=".$_REQUEST['productprice'].", productdprice=".$dprice.", productimagepath='".$_REQUEST['productimagepath']."' where productcode='".$_REQUEST['productcode']."'";
    $try_to_update_product = $connect->query($query);
    if (!$try_to_update_product) {
      $statusMsg = "Product update failed at index.php page error code : ".$connect->errno." query : ".$query;
    }
