...
if ($session['session_valid'] && $session['auth_key_valid']) {
  $retreive_all_billinglist = $connect->query("select * from stockrequest where status='accept' or status='refund'");
    ...
    while ($billing_row = $retreive_all_billinglist->fetch_assoc()) {
      ...
      <div class="tabcontent__field">
        ...
        <span class="tabcontent__action">
          ...
          <button class="button__icon button__green" name="mark" data-key="<?php echo $billing_row['keyhash']; ?>"><i class="fas fa-check"></i>Mark as done</button>
        </span>
      </div>
      <?php
    }
